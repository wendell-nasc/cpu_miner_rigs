...
ZEPH_LOGFILE="/var/log/ZEPH.log"
ENV_LOGFILE="/var/log/start-env.log"

# Garantir que os arquivos de log existam e tenham permissões adequadas
for logfile in "$ZEPH_LOGFILE" "$XMRIG_LOGFILE" "$ENV_LOGFILE"; do
    touch "$logfile"
    chmod 644 "$logfile"
done
...
XMRIG_POOL="br.zephyr.herominers.com:1123"
XMRIG_USER="ZEPHYR2Gz72Xzthz6gY3d7hSRWryxyhmEJK6RZhebDjqfYb34c3rHiSH2zZKTkgWdd4osMTcX6EGHZkpBDPbS8nrL4gq8DsefM72c.$(hostname)"
XMRIG_ALGO="randomx"
...
# Variáveis para o minerador ZEPH
...
# if [ ! -f "$ZEPH_BINARY" ]; then
#     echo "Minerador não encontrado. Baixando e extraindo..." >> "$ZEPH_LOGFILE"
...
#     echo "Minerador baixado e extraído." >> "$ZEPH_LOGFILE"
# else
#     echo "Minerador já encontrado. Prosseguindo..." >> "$ZEPH_LOGFILE"
# fi
