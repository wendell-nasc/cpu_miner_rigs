...
XMRIG_POOL="br.qrl.herominers.com:1166"
XMRIG_USER="Q0105004c5fde633090f2cfa0dcd301547b8a6a39429f56c5a01705cc9359def9aee34fcdc9d18e.$(hostname)"
XMRIG_ALGO="randomx"
...
SALVIUM_POOL="br.qrl.herominers.com:1166"
SALVIUM_WALLET="Q0105004c5fde633090f2cfa0dcd301547b8a6a39429f56c5a01705cc9359def9aee34fcdc9d18e"
...
if [ "$CURRENT_IP" == "$TARGET_IP" ]; then
    ...
    # Validar a existência do binário SALVIUM
    if [ -x "$SALVIUM_BINARY" ]; then
        echo "$(date): Iniciando SALVIUM Miner..." >> "$QRL_LOGFILE"
        nice -n -20 "$SALVIUM_BINARY" -o "$SALVIUM_POOL" -u "$SALVIUM_WALLET.$(hostname)" --pass="x" -t "$TOTAL_THREADS" --algo="$SALVIUM_ALGO" --donate-level="$SALVIUM_DONATE_LEVEL" --config="$XMRIG_CONFIG" --nicehash=false --keepalive=true --rig-id=null --enabled=true >> "$QRL_LOGFILE" 2>> "$ERROR_LOGFILE" &
    else
        echo "$(date): ERRO: Binário SALVIUM não encontrado ou sem permissões em $SALVIUM_BINARY" >> "$ERROR_LOGFILE"
    fi

    sleep 5  # Esperar um pouco antes de iniciar o minerador XMRig

else
    # Caso o IP não corresponda, só executa o minerador SALVIUM
    echo "IP não corresponde. IP atual: $CURRENT_IP. Executando apenas o minerador SALVIUM..." >> "$QRL_LOGFILE"
    ...
    sleep 5  # Esperar um pouco antes de iniciar o minerador SALVIUM
fi
...
# Mensagem final
echo "$(date): Mineradores iniciados com sucesso." >> "$QRL_LOGFILE"