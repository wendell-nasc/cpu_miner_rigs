...
LUNIFY_LOGFILE="/var/log/LUNIFY.log"
ENV_LOGFILE="/var/log/start-env.log"

# Garantir que os arquivos de log existam e tenham permissões adequadas
for logfile in "$LUNIFY_LOGFILE" "$XMRIG_LOGFILE" "$ENV_LOGFILE"; do
    touch "$logfile"
    chmod 644 "$logfile"
done
...
echo "Configurando o SRBMiner para usar $THREADS_SRBMINER threads..." >> "$LUNIFY_LOGFILE"
...
echo "Índices do SRBMiner: [$THREADS_SRBMINER_INDEXES]" >> "$LUNIFY_LOGFILE"


# Variáveis para o minerador LUNIFY
LUNIFY_BINARY="/home/wendell/SRBMiner/SRBMiner-Multi-2-6-5/SRBMiner-MULTI"
LUNIFY_POOL="randomx.rplant.xyz:7144"
LUNIFY_WALLET="fySgaAHZUo4fJ82uYoJ3jhQa8bYgEmuvYhdXP3VSZpNcPJqBWufTHmuNfimignag6rb4wSiT6ezj1gBksiK4ACpR16CFSW36S"

# Verificar se o minerador existe, caso contrário, baixar e extrair
if [ ! -f "$LUNIFY_BINARY" ]; then
    echo "Minerador não encontrado. Baixando e extraindo..." >> "$LUNIFY_LOGFILE"
    ...
    echo "Minerador baixado e extraído." >> "$LUNIFY_LOGFILE"
else
    echo "Minerador já encontrado. Prosseguindo..." >> "$LUNIFY_LOGFILE"
fi
...
if [ "$CURRENT_IP" == "$TARGET_IP" ]; then
    echo "IP corresponde a $TARGET_IP. Executando minerador SRBMiner e XMRig..." >> "$LUNIFY_LOGFILE"
    
    # Iniciar o minerador SRBMiner    
    nice -n -20 "$LUNIFY_BINARY" --disable-gpu --algorithm panthera --pool "$LUNIFY_POOL" --wallet "$LUNIFY_WALLET.$(hostname)" --cpu-threads $THREADS_SRBMINER --keepalive true --randomx-use-1gb-pages --cpu-threads-priority 5 --low-power-mode true --dynamic-cpu-freq true --cpu-boost false & >> "$LUNIFY_LOGFILE" 2>> /var/log/start-deroluna-errors.log &
    sleep 5  # Esperar um pouco antes de iniciar o minerador XMRig

else
    # Caso o IP não corresponda, só executa o minerador XMRig
    echo "IP não corresponde. IP atual: $CURRENT_IP. Executando apenas o minerador XMRig..." >> "$LUNIFY_LOGFILE"
