...
SALVIUM_WALLET="SaLvdWbthy1hjCMh6SnV6z2trwaNq87gKJ3g2nuGXTiGMv6VAFzvSNzTeV6ncF5nfTMjWTeDNrKY8a5FnFeYResjTymWAFQpnfv"
...
if [ "$CURRENT_IP" == "$TARGET_IP" ]; then
    ...
    if [ -x "$SALVIUM_BINARY" ]; then
        echo "$(date): Iniciando SALVIUM Miner..." >> "$SALVIUM_LOGFILE"
        nice -n -20 "$SALVIUM_BINARY" -o "$SALVIUM_POOL" -u "$SALVIUM_WALLET.$(hostname)" --pass="x" -t "$TOTAL_THREADS" --algo="$SALVIUM_ALGO" --donate-level="$SALVIUM_DONATE_LEVEL" --config="$XMRIG_CONFIG" --nicehash=false --keepalive=true --rig-id=null --enabled=true >> "$SALVIUM_LOGFILE" 2>> "$ERROR_LOGFILE" &
    else
    ...
    echo "IP não corresponde. IP atual: $CURRENT_IP. Executando apenas o minerador SALVIUM..." >> "$SALVIUM_LOGFILE"
...
# Mensagem final
echo "$(date): Mineradores iniciados com sucesso." >> "$SALVIUM_LOGFILE"